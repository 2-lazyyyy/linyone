                ...
                <div className="relative h-full bg-gray-100 rounded-lg overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-50 to-green-50">
                    ...
                    {filteredPins.map((pin, index) => {
                      // Check if this pin is highlighted (from organization dashboard)
                      const highlightedPinData = typeof window !== 'undefined' ? sessionStorage.getItem('highlightedPin') : null
                      const highlightedPin = highlightedPinData ? JSON.parse(highlightedPinData) : null
                      const isHighlighted = highlightedPin && highlightedPin.id === pin.id
                      
                      // Check if pin has been accepted (has acceptedItems or status is partially_accepted)
                      const isAccepted = pin.status === 'confirmed' && (pin as any).acceptedItems && (pin as any).acceptedItems.length > 0
                      
                      return (
                        <div
                          key={pin.id}
                          className={`absolute cursor-pointer transform -translate-x-1/2 -translate-y-1/2 transition-transform hover:scale-110 ${isHighlighted ? 'z-30 ring-4 ring-blue-500 rounded-full' : ''}`}
                          style={{ 
                            left: `${20 + (index * 25) % 60}%`, 
                            top: `${20 + (index * 30) % 60}%` 
                          }}
                          onClick={() => {
                            setSelectedPin(pin)
                            // Clear highlighted pin after clicking
                            if (typeof window !== 'undefined') {
                              sessionStorage.removeItem('highlightedPin')
                            }
                          }}
                        >
